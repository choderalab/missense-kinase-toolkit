...
from sklearn.cluster import DBSCAN, SpectralClustering, KMeans # noqa: F401
...
def find_kmeans(
    ...
    kl = KneeLocator(range(1, n_clust+1), list_sse, curve="convex", direction="decreasing")
    n_clust = kl.elbow
...
def plot_dim_red_scatter(
    ...
    patches = [mpatches.Patch(color=cmap(i), label=str(i + 1)) for i in range(n_clusters)]
...
def scatter_plot_binary_grid(
    ...
    filtered_families = {fam: count for fam, count in dfs_dict.items() if count >= n_cutoff}
    ...
    fig.tight_layout(pad=4.0, rect=[0, 0, 1, 0.975])  # Adjust rect to leave space for suptitle
    ...
    n_clusters = len(np.unique(kmeans.labels_))
    cmap = plt.get_cmap("Set1", n_clusters)
    axes[0].scatter(df_tsne["tSNE1"], df_tsne["tSNE2"], c=kmeans.labels_ + 1, cmap=cmap, alpha=0.5)
    ...
    for i, (fam, count) in enumerate(filtered_families.items()):
        if i+1 < len(axes):  # +1 because we used the first position for kmeans
            ...
            ax.scatter(df_tsne["tSNE1"], df_tsne["tSNE2"], c=vx_fam, cmap=cmap_bin, alpha=0.5)
            ax.set_xlabel("tSNE1")
            ax.set_ylabel("tSNE2")
            ax.set_title(f"{fam} (n={count})")
    
    # Hide any unused subplots
    for j in range(i+2, len(axes)):  # +2 because we used the first position for kmeans
        axes[j].axis('off')
...
#TODO: automate this
df_annot = pd.read_csv("/data1/tanseyw/projects/whitej/missense-kinase-toolkit/data/pkis2_annotated_group.csv")
list_to_drop = ["-phosphorylated", "-cyclin", "-autoinhibited"]
idx_drop = df_annot["DiscoverX Gene Symbol"].apply(lambda x: any([i in x for i in list_to_drop]))
df_annot = df_annot.loc[~idx_drop, :].reset_index(drop=True)
df_annot = df_annot.loc[df_annot["sequence_partial"].notnull(), :].reset_index(drop=True)
...
col = "group"
df_annot.loc[~df_annot[col].isnull(), col] = df_annot.loc[~df_annot[col].isnull(), col].apply(literal_eval)
set_annot = set(chain(*df_annot.loc[~df_annot[col].isnull(), col].tolist()))
dict_annot = {entry: sum([entry in i for i in df_annot.loc[~df_annot[col].isnull(), col]]) for entry in set_annot}
