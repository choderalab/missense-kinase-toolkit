...
class TensorFactorPipeline(TensorFactorTrainer):
    ...
    def __post_init__(self):
        ...
        super().__post_init__()
        print(
            f"    Optimizer: {self.optimizer.__class__.__name__}, LR={self.learning_rate}"
        )
    ...
    def generate_split_dataframe(self) -> pd.DataFrame:
        ...
        """
        df_split = (
            self.df.sample(frac=1, random_state=self.seed).reset_index(drop=True).copy()
        )
        ...
        df_split[self.val_col] = np.logical_and.reduce(
            [
                df_split[k].isin([v["dict_map"][i] for i in v["list_idx"]])
                for k, v in dict_splits.items()
            ]
        )
        df_split.insert(
            len(df_split.columns) - 1, self.target_col, df_split.pop(self.target_col)
        )
    ...
    def create_dataloader_from_df(self) -> tuple[DataLoader, DataLoader]:
        ...
        train_targets = torch.tensor(
            df_train[self.target_col].values, dtype=torch.float32
        )
        val_targets = torch.tensor(df_val[self.target_col].values, dtype=torch.float32)

        # create datasets
        train_dataset = TensorDataset(
            train_mask.unsqueeze(0), train_targets.unsqueeze(0)
        )
