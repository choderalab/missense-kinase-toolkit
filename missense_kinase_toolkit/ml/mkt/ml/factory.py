...
from mkt.ml.constants import DICT_MODELS, DICT_DATASET
...
class ExperimentFactory:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            if not all(
                key in self.config for key in ["data", "model", "trainer"]
            ):
                raise ValueError("Configuration file must contain 'data', 'model' and 'trainer' keys.")
    ...
    def create_experiment(self) -> tuple[object | None, object | None]:
        ...
        try:
            ...
            if model_class is None:
                raise ValueError(f"Model {self.config["model"]["model_type"]} not found.")
