...
def create_dataloaders(
    ...
    def collate_fn(batch):
        ...
        klifs_attention_mask = torch.stack([item["klifs_attention_mask"] for item in batch])
...
def evaluate_model_with_wandb(
    ...
    plt.plot([min(all_labels_original), max(all_labels_original)], 
    ...
    for i in range(min(len(all_labels_original), 100)):  # Limit to 100 examples
        data.append([i, float(all_labels_original[i][0]), float(all_preds_original[i][0])])
