...
def main():
    ...
    encode_val = [
        dataset.map(
            lambda x: tokenizer(x[col_seq], **dict_token_args), 
            batched=True
        ) \
        for dataset in ds_val
    ]
    encode_val = [
        dataset.remove_columns([col_seq]) \
        for dataset in encode_val
    ]
    encode_train = [
        dataset.map(
            lambda x: tokenizer(x[col_seq], **dict_token_args), 
