...
def extract_pk_fasta_info_as_dict(
    ...
    """
    fasta_sequences = SeqIO.parse(open(os.path.join(get_repo_root(), "data", str_filename)),'fasta')
...
def align_kincore2uniprot(
    ...
    if alignment.sequences[0] != alignment[0, :]:
        print(f"Alignment does not include full sequence \
    ...
    if "." in str_align:
        dict_out["mismatch"] = [idx for idx, i in \
                                enumerate(str_align) if i == "."]
