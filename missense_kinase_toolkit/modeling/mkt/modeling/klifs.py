...
class KLIFSIFPGenerator:
    ...
    def extract_klifs_pocket(
        self, universe: mda.Universe, klifs_residues: list[tuple[int, str]]
    ) -> mda.AtomGroup:
        """
    ...
    def generate_ifp(
        ...
        ifp = self.fp.run_from_iterable([u], protein_sel=pocket, ligand_sel=ligand)
    ...
    def _create_klifs_binary_vector(
        self, ifp_df: pd.DataFrame, klifs_residues: list[tuple[int, str]]
    ) -> np.ndarray:
        """
        ...
        for klifs_idx, (resnum, chain) in enumerate(klifs_residues):
            ...
            if not residue_found:
                print(
                    f"Warning: KLIFS residue {klifs_idx+1} ({resnum}:{chain}) not found in IFP"
                )

        return binary_vector

    def _detect_edge_to_face_interaction(
        self, ifp_df: pd.DataFrame, residue_key: str, klifs_idx: int
    ) -> int:
        """
    ...
    def _save_results(
        self, ifp_df: pd.DataFrame, binary_vector: np.ndarray, output_prefix: str
    ):
        ...
        for i in range(85):
            for prop in self.klifs_properties:
                klifs_labels.append(
                    f"Res{i+1:02d}_Prop{prop['position']}_{prop['name'].replace(' ', '_')}"
                )
    ...
    def batch_process(
        ...
        for i, (cif_file, klifs_residues) in enumerate(
            zip(cif_files, klifs_residues_list)
        ):
            try:
                ...
                # Store result
                result_dict = {"complex_id": Path(cif_file).stem, "cif_file": cif_file}
...
def main():
    ...
    try:
        binary_ifp = generator.generate_ifp(
            cif_file, klifs_residues, output_prefix="example_output"
        )

        print(
            f"Generated KLIFS binary IFP with {len(binary_ifp)} bits (85 residues × 7 properties)"
        )
